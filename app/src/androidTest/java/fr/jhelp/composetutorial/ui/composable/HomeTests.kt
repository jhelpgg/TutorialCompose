...
class HomeTests
{
    companion object
    {
        val homeDataMutable = ObservableSource<HomeData>(HomeData(information = "First"))
        ...
        fun initialize()
        {
            ...
            provideSingle<HomeModel> {
                HomeDummy(HomeTests.homeDataMutable.observable)
                { homeAction ->
                    when (homeAction)
                    {
                        is HomeActionChangeInformation ->
                            HomeTests.homeDataMutable.value = HomeData(information = homeAction.information)

                        is HomeActionCallData          ->
                            HomeTests.homeDataMutable.value = HomeData(information = "Second")
                    }
                }
            }
        }
